...
import {
  ...
  Youtube, TrendingUp, Globe, MapPin, X, Building2, ShoppingBag, Compass, CreditCard,
...
const TikTokIcon = ({ className, size = 16, style }: { className?: string; size?: number; style?: React.CSSProperties }) => (
  <svg
    ...
    style={style}
  >
    <path d="M19.59 6.69a4.83 4.83 0 0 1-3.77-4.25V2h-3.45v13.67a2.89 2.89 0 0 1-5.2 1.74 2.89 2.89 0 0 1 2.31-4.64 2.93 2.93 0 0 1 .88.13V9.4a6.84 6.84 0 0 0-1-.05A6.33 6.33 0 0 0 5 15.64a6.34 6.34 0 0 0 10.86 4.27 6.34 6.34 0 0 0 1.93-4.58V9.69a7.87 7.87 0 0 0 4.59 1.36V8.02a4.5 4.5 0 0 1-2.79-1.33z" />
  </svg>
);

/* --- Global Styles (no glassmorphism – solid backgrounds only) --- */
const G = () => <style>{`
...
@keyframes navIconFloat{0%,100%{transform:translateY(0)}50%{transform:translateY(-2px)}}
...
/* --- Interactive Star Field (mouse parallax, high performance) --- */
const StarField = () => {
  ...
  useEffect(() => {
    ...
    let stars: Array<{
      ...
      r: number, color: string, alpha: number, speed: number
    }> = [];
    ...
    const draw = () => {
      ...
      for (const s of stars) {
        let drawX = s.baseX + offsetX * (s.r / 2);
        let drawY = s.baseY + offsetY * (s.r / 2);
        if (drawX < -20) drawX = width + 20;
        if (drawX > width + 20) drawX = -20;
        if (drawY < -20) drawY = height + 20;
        if (drawY > height + 20) drawY = -20;
...
/* --- Grain Overlay (very subtle, no glass) --- */
const GrainOverlay = () => (
  <svg className="pointer-events-none fixed inset-0 w-full h-full" style={{ zIndex: 2, opacity: 0.03 }} xmlns="http://www.w3.org/2000/svg">
    <filter id="grain-f"><feTurbulence type="fractalNoise" baseFrequency="0.75" numOctaves="3" stitchTiles="stitch" /><feColorMatrix type="saturate" values="0" /></filter>
    <rect width="100%" height="100%" filter="url(#grain-f)" />
  </svg>
);

/* --- Border Beam (no glass) --- */
const BorderBeam = ({ color = '#a855f7', size = 1.5, dur = 3 }: { color?: string; size?: number; dur?: number }) => (
  <div className="absolute inset-0 rounded-2xl overflow-hidden pointer-events-none" style={{ zIndex: 1 }}>
    ...
    />
    <style>{`
      .border-beam { position: absolute; inset: 0; border-radius: inherit; -webkit-mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0); mask: linear-gradient(#fff 0 0) content-box, linear-gradient(#fff 0 0); -webkit-mask-composite: xor; mask-composite: exclude; }
...
/* --- Atom Orbit with proper social logos (Instagram, TikTok, YouTube) --- */
const AtomOrbit = () => {
  const SZ = 500;
  const C  = SZ / 2;
  const R1 = 130, R2 = 220;
  ...
  const inner = [
    { deg: -90, color: '#E1306C', label: 'Instagram', icon: <Instagram size={18} style={{ color: '#E1306C' }} /> },
    { deg:  30, color: '#69C9D0', label: 'TikTok',    icon: <TikTokIcon size={16} style={{ color: '#69C9D0' }} /> },
    { deg: 150, color: '#FF0000', label: 'YouTube',   icon: <Youtube size={18} style={{ color: '#FF0000' }} /> },
  ];

  const outer = [
    { deg: -90, color: '#a855f7', label: 'AI Search',    icon: <Search    size={14} style={{ color: '#c084fc' }} /> },
    { deg: -18, color: '#60a5fa', label: 'Fraud Check',  icon: <Shield    size={14} style={{ color: '#60a5fa' }} /> },
    { deg:  54, color: '#4ade80', label: 'ROAS Engine',  icon: <TrendingUp size={14} style={{ color: '#4ade80' }} /> },
    { deg: 126, color: '#facc15', label: 'Live Data',    icon: <Zap       size={14} style={{ color: '#facc15' }} /> },
    { deg: 198, color: '#fb923c', label: 'Multi-City',   icon: <MapPin    size={14} style={{ color: '#fb923c' }} /> },
    { deg: 270, color: '#f472b6', label: 'Team Board',   icon: <Users     size={14} style={{ color: '#f472b6' }} /> },
  ];
  ...
  const ring = (W: number, dur: string, cls: string, dir: 'cw' | 'ccw', nodes: typeof inner, iconSz: number) => (
    <div style={{
      position: 'absolute', top: '50%', left: '50%',
      width: W, height: W, marginTop: -W / 2, marginLeft: -W / 2,
      borderRadius: '50%',
      border: `1px solid rgba(168,85,247,${dir === 'cw' ? '0.2' : '0.12'})`,
...
/* --- Marquee & BentoCard (solid, no blur) --- */
    ...
    <motion.div ref={ref} whileHover={{ scale: 1.02, y: -3 }} transition={{ duration: .18 }}
      onMouseMove={e => {
        ...
        if (r && spotRef.current) {
          ...
          spotRef.current.style.background = `radial-gradient(280px at ${x}px ${y}px,rgba(168,85,247,0.12),transparent 60%)`;
...
/* --- SectionSpotlight (mouse-follow glow, no glass) --- */
const SectionSpotlight = ({ children, id, className = '', 'aria-label': ariaLabel }: any) => {
  const sRef = useRef<HTMLElement>(null);
  const spotRef = useRef<HTMLDivElement>(null);
  return (
    <section ref={sRef} id={id} aria-label={ariaLabel} className={`relative overflow-hidden ${className}`}
      onMouseMove={e => {
        const r = sRef.current?.getBoundingClientRect();
        if (r && spotRef.current) {
          spotRef.current.style.background = `radial-gradient(700px at ${e.clientX - r.left}px ${e.clientY - r.top}px,rgba(168,85,247,0.07),transparent 65%)`;
          spotRef.current.style.opacity = '1';
        }
      }}
      onMouseLeave={() => { if (spotRef.current) spotRef.current.style.opacity = '0'; }}>
      <div ref={spotRef} aria-hidden="true" className="pointer-events-none absolute inset-0" style={{ opacity: 0, transition: 'opacity 0.35s ease', zIndex: 1 }} />
      <div className="relative z-10">{children}</div>
    </section>
  );
};
...
const MockSearch = () => (
  <div className="rounded-2xl border border-white/10 overflow-hidden bg-[#0d0d14] shadow-2xl select-none text-xs">
    ...
    <div className="p-4 space-y-3">
      <div className="flex gap-2 items-center bg-[#0f1119] rounded-lg px-3 py-2.5 border border-white/10">
      ...
      <div className="flex gap-2">
        {['Travel', 'Karachi', 'Instagram'].map(f => <span key={f} className="px-2.5 py-1 rounded-full bg-[#0f1119] border border-white/10 text-white/60">{f}</span>)}
      </div>
...
export default function LandingPage() {
  ...
  useMotionValueEvent(scrollYProgress, 'change', (v) => { if (v >= 0.98) setHeroComplete(true); });

  const navItems = [
    { id: 'features', icon: <Compass className="w-4 h-4" />, label: 'Features' },
      ...
      <div className="bg-[#060608] text-white">
        ...
        {heroComplete && (
          <motion.div initial={{ opacity: 0, y: 24 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.75 }}>
            ...
            {/* Built For section */}
            <SectionSpotlight className="py-16 px-6 border-b border-white/[0.06]">
              <div className="max-w-5xl mx-auto">
                <div className="text-center mb-10">
                  <motion.div initial={{ opacity: 0, x: -16 }} whileInView={{ opacity: 1, x: 0 }} className="text-purple-400 text-xs font-medium uppercase tracking-widest mb-4">Built For</motion.div>
                  <RevealLine />
                  <h2 className="text-3xl md:text-4xl font-black tracking-tighter leading-tight">
                    <RevealText text="Every Team That Needs" /> <RevealText text="Real Results." className="shiny-text" delay={0.18} />
                  </h2>
                </div>
                <div className="grid sm:grid-cols-2 md:grid-cols-4 gap-4">
                  {[
                    ...
                    <motion.div key={item.title} initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.08 }}
                      className="group rounded-2xl border border-white/[0.08] bg-[#0f1119] p-6 hover:border-purple-500/20 hover:bg-[#131625] transition-all">
                      <div className="mb-4 w-10 h-10 rounded-xl bg-[#0f1119] border border-white/10 flex items-center justify-center">{item.icon}</div>
            ...
            {/* Bento Stats */}
            <SectionSpotlight className="py-20 px-6 border-b border-white/[0.06]">
              <div className="max-w-5xl mx-auto">
                <div className="text-center mb-10">
            ...
            {/* Platforms */}
            <SectionSpotlight id="features" className="py-24 px-6">
              <div className="max-w-5xl mx-auto">
                ...
                <motion.div initial={{ opacity: 0, y: 24 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="rounded-2xl border border-white/10 overflow-hidden">
                  <div className="overflow-x-auto">
                    <table aria-label="Mushin vs alternatives feature comparison" className="w-full text-sm min-w-[520px]">
                      <thead>
                        <tr className="border-b border-white/10 bg-[#0f1119]">
                      ...
                      <tbody>
                        {([
                          ...
                          <tr key={i} className={`border-b border-white/10 last:border-0 ${i % 2 !== 0 ? 'bg-[#0d0f15]' : ''}`}>

...
export function PricingPreview({ ctaPath }: PricingPreviewProps) {
  return (
          ...
          {(Object.entries(PLANS) as [string, typeof PLANS.free][]).map(([key, plan]) => {
            ...
            return (
              <motion.div
                key={key}
                variants={fadeUp}
                className={`glass-card rounded-xl p-8 text-left space-y-6 relative will-change-transform transition-all duration-200 hover:scale-[1.02] hover:shadow-lg ${isPro ? "ring-2 ring-primary" : ""}`}
              >
                {isPro && (

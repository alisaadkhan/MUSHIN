        ...
        <div className="flex flex-col md:flex-row justify-between gap-8 mb-10">
          <div className="max-w-xs">
            ...
            <p className="text-zinc-500 text-sm leading-relaxed">
              Pakistan's first AI-powered influencer intelligence platform. Built to bring signal to a noisy market.
            </p>
          </div>
            ...
            <div>
              ...
              <div className="space-y-2">
                {['Features','Pricing','API','Changelog'].map(l => (
                  <div key={l} className="text-zinc-500 hover:text-white transition-colors cursor-pointer">{l}</div>
                ))}
              </div>
            </div>
            <div>
              ...
              <div className="space-y-2">
                {['About','Blog','Careers','Press'].map(l => (
                  <Link key={l} to={l === 'About' ? '/about' : l === 'Blog' ? '/blog' : '#'} className="block text-zinc-500 hover:text-white transition-colors">
                    {l}
                  </Link>
                ))}
              </div>

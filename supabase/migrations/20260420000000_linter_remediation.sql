...
ALTER FUNCTION public.notify_security_anomaly()
  SET search_path = '';
-- locks function to fully-qualified names only
...
-- allow any role to read active announcements
...
DROP INDEX IF EXISTS public.idx_ip_tags;
-- keep idx_ip_tags_gin (GIN)

-- influencers_cache tags: same pattern
DROP INDEX IF EXISTS public.idx_ic_tags;
-- keep idx_ic_tags_gin (GIN)
...
DROP INDEX IF EXISTS public.idx_linter_wm_workspace_id;
-- not a duplicate pair, just unused
...
DROP INDEX IF EXISTS public.idx_enrichment_jobs_workspace;
-- safe: duplicate was already dropped
